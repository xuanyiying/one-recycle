...
export const saveDraftOrder = (
  ...
  try {
    ...
    localStorage.setItem(DRAFT_ORDER_STORAGE_KEY, draftJson)
    return draft
  } catch (error) {
...
export const updateDraftOrder = (
  ...
  try {
    const draftJson = JSON.stringify(updatedDraft)
    localStorage.setItem(DRAFT_ORDER_STORAGE_KEY, draftJson)
    return updatedDraft
  } catch (error) {
...
export const getDraftOrder = (): DraftOrder | null => {
  try {
    const draftJson = localStorage.getItem(DRAFT_ORDER_STORAGE_KEY)
