...
class AccountService {
  ...
  async getMyAccount(): Promise<Account> {
    try {
      // Fetch from API
      const account = await get('/api/accounts/me', undefined, { cache: true, cacheTTL: CacheTTL.MEDIUM });
      ...
      if (cached) {
        console.log('Returning cached account data due to API error');
        return cached;
      }
      throw error;
    }
  ...
  async getMyTransactions(
    ...
    return get('/api/accounts/me/transactions', params);
  }
  ...
  async getMyStats(): Promise<AccountStats> {
    return get('/api/accounts/me/stats');
  }
