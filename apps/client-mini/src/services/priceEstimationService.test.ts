...
const mockPricing: OrderPricing = {
  ...
  serviceFee: 5,
  totalEstimate: { min: 105, max: 155, currency: 'CNY' },
  breakdown: [
    {
      itemId: '1',
...
describe('Price Estimation Service', () => {
  ...
  describe('validateItemsForPricing', () => {
    ...
    it('should return error for invalid weight', () => {
      const invalidItem = { ...mockItem, weight: 0 }
      const errors = validateItemsForPricing([invalidItem])
      expect(errors.some((e) => e.includes('重量必须大于0'))).toBe(true)
    })

    it('should return error for invalid quantity', () => {
      const invalidItem = { ...mockItem, quantity: -1 }
      const errors = validateItemsForPricing([invalidItem])
      expect(errors.some((e) => e.includes('数量必须大于0'))).toBe(true)
    })
