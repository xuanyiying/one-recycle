...
export default function Login() {
  ...
  const handleLoginSuccess = () => {
    if (process.env.NODE_ENV === 'development') {
      console.log('[Login] Login success, redirecting to:', redirectUrl)
    }
      ...
      <View className="main-content">
        ...
        {loginMode === 'quick' && platform !== 'unknown' && (
          <View className="quick-login-section">
            <Button
              className={`authorize-btn ${platform} ${loading ? 'loading' : ''}`}
              type="primary"
              block
              loading={loading}
              onClick={onSocialLogin}
              disabled={loading || !agreedToTerms}
              style={{ backgroundColor: platform === 'wechat' ? '#07C160' : '#1677FF', borderColor: 'transparent' }}
            >
        ...
        {(platform === 'wechat' || platform === 'alipay') && (
          <View className="switch-mode" onClick={switchMode}>
            <Text className="switch-text">
              {loginMode === 'quick' ? '手机号登录' : `${platform === 'wechat' ? '微信' : '支付宝'}一键登录`}
            </Text>
          </View>
        )}
