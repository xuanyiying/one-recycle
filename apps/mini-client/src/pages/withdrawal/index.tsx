      ...
      <ScrollView className='wallet-page' scrollY>
        ...
        <View className='asset-card'>
          <View className='card-header'>
            ...
            <View className='rules' onClick={() => Taro.navigateTo({ url: '/pages/points-rules/index' })}>
              <Icon name='tips' size={12} color='rgba(255,255,255,0.8)' />
              <Text>规则</Text>
            </View>
          </View>

          <View className='points-display'>
            <Text className='points'>¥{Number(account?.availableBalance ?? 0).toFixed(2)}</Text>
            <Text className='value'>冻结: ¥{Number(account?.frozenBalance ?? 0).toFixed(2)}</Text>
          </View>
        ...
        <View className='menu-grid'>
          <View className='menu-item' onClick={() => Taro.navigateTo({ url: '/pages/transaction/list/index' })}>
            <View className='icon-box blue'><Icon name='money' size={20} /></View>
            <Text>收支明细</Text>
          </View>
          <View className='menu-item' onClick={() => Taro.navigateTo({ url: '/pages/withdrawal/list/index' })}>
            <View className='icon-box orange'><Icon name='order' size={20} /></View>
            <Text>提现记录</Text>
          </View>
          <View className='menu-item' onClick={() => Taro.navigateTo({ url: '/pages/customer/index' })}>
            <View className='icon-box green'><Icon name='service' size={20} /></View>
            <Text>联系客服</Text>
          </View>
        </View>
        ...
        <View className='transaction-section'>
          <View className='section-header'>
            ...
            <View className='more' onClick={() => Taro.navigateTo({ url: '/pages/transaction/list/index' })}>
              <Text>查看全部</Text>
        ...
        >
          <View className='withdraw-popup' style={{ paddingBottom: safeAreaBottom > 0 ? `${safeAreaBottom}px` : undefined }}>
            ...
            <View className='balance-tip'>
              当前可提现余额 ¥{Number(account?.availableBalance ?? 0).toFixed(2)}，
