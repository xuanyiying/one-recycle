    ...
    useEffect(() => {
        const loadTimeSlots = async () => {
            ...
            try {
                ...
                if (!addressId) {
                    logger.warn('No address selected, using fallback time slots')
                ...
                logger.error('Failed to fetch time slots from API:', error)
        ...
        return () => {
            if (reservation && selectedSlotId) {
                releaseTimeSlot(selectedSlotId).catch(error => {
                    logger.error('Failed to release time slot on unmount:', error)
                })
            }
        }
    ...
    const handleDateChange = useCallback(async (date: string) => {
        ...
        if (addressId && !timeSlots.some(slot => slot.date === date)) {
            ...
            try {
                ...
                logger.error('Failed to fetch time slots for date:', error)
