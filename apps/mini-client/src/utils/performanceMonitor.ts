...
class PerformanceMonitor {
    ...
    trackPageLoad(pagePath: string, startTime: number) {
        ...
        // Log slow page loads
        if (loadTime > 1000) {
            logger.warn(`Slow page load: ${pagePath} took ${loadTime}ms`);
        }
    ...
    getSummary() {
        try {
            ...
            logger.error('[PerformanceMonitor] Failed to get summary:', error);
            return {
    ...
    async sendToAnalytics() {
        try {
            ...
            logger.error('[PerformanceMonitor] Failed to send to analytics:', error);
        }
    }
}
