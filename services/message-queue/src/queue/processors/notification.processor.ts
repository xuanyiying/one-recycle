...
export class NotificationProcessor {
  ...
  async handleSendSms(job: Job<SmsNotificationEventDto>): Promise<any> {
    ...
    try {
      ...
      const result = await this.notificationServiceClient.sendSms({
        phone,
        template,
        params,
      });
  ...
  async handleSendPush(job: Job<PushNotificationEventDto>): Promise<any> {
    ...
    try {
      ...
      const result = await this.notificationServiceClient.sendPush({
        userId,
        title,
        content,
        data,
  ...
  async handleSendEmail(job: Job<EmailNotificationEventDto>): Promise<any> {
    ...
    try {
      ...
      const result = await this.notificationServiceClient.sendEmail({
        to,
        subject,
        content,
        template,
  ...
  async handleBatchNotification(job: Job<BatchNotificationEventDto>): Promise<any> {
    ...
    try {
      ...
      const result = await this.notificationServiceClient.sendBatch(type, notifications);
  ...
  async handleWithdrawalCreated(job: Job<WithdrawalCreatedEventDto>): Promise<any> {
    ...
    try {
      // 1. 获取用户手机号
      const userPhone = await this.notificationServiceClient.getUserPhone(userId);
      
      // 2. 发送短信通知用户提现申请已提交
      if (userPhone) {
        this.logger.log(`Sending SMS notification for withdrawal created: ${withdrawalId}`);
        await this.notificationServiceClient.sendSms({
          phone: userPhone,
      ...
      await this.notificationServiceClient.sendPush({
        userId,
        title: '提现申请已提交',
