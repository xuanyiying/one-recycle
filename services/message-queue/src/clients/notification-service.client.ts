...
export class NotificationServiceClient {
  ...
  constructor(private readonly configService: ConfigService) {
    this.baseURL =
      this.configService.get<string>('NOTIFICATION_SERVICE_URL') || 'http://localhost:3004';
  ...
  private setupInterceptors(): void {
    this.httpClient.interceptors.request.use(
      (config) => {
        this.logger.debug(`Notification request: ${config.method?.toUpperCase()} ${config.url}`);
        return config;
      },
      (error) => {
  ...
  async sendSms(request: SendSmsRequest): Promise<SendSmsResponse> {
    try {
      this.logger.log(`Sending SMS to ${request.phone}, template: ${request.template}`);

      const response = await this.httpClient.post<SendSmsResponse>('/notifications/sms', request);
  ...
  async sendPush(request: SendPushRequest): Promise<SendPushResponse> {
    try {
      ...
      const response = await this.httpClient.post<SendPushResponse>('/notifications/push', request);
      ...
      this.logger.error(`Failed to send push notification to user ${request.userId}:`, error);
      throw error;
    }
  }
   ...
   * 批量发送通知
   */
  async sendBatch(type: 'sms' | 'push' | 'email', notifications: any[]): Promise<any> {
    try {
      this.logger.log(`Sending batch ${type} notifications: ${notifications.length} items`);

      const response = await this.httpClient.post(`/notifications/batch/${type}`, {
        notifications,
      });
  ...
  async getUserPhone(userId: string): Promise<string | null> {
    try {
      const response = await this.httpClient.get<{ phone: string }>(`/users/${userId}/phone`);
  ...
  async getUserEmail(userId: string): Promise<string | null> {
    try {
      const response = await this.httpClient.get<{ email: string }>(`/users/${userId}/email`);
