...
export class DispatchServiceClient {
  ...
  constructor(private readonly configService: ConfigService) {
    this.baseURL = this.configService.get<string>('DISPATCH_SERVICE_URL') || 'http://localhost:3006';
  ...
  private setupInterceptors(): void {
    this.httpClient.interceptors.request.use(
      (config) => {
        this.logger.debug(`Dispatch request: ${config.method?.toUpperCase()} ${config.url}`);
        return config;
      },
      (error) => {
  ...
  async autoDispatch(request: DispatchRequest): Promise<DispatchResponse> {
    try {
      ...
      this.logger.error(`Failed to auto dispatch order ${request.orderId}:`, error);
      throw error;
    }
  }
   ...
   * 手动派单
   */
  async manualDispatch(orderId: string, courierId: string): Promise<DispatchResponse> {
    try {
      this.logger.log(`Manually dispatching order ${orderId} to courier ${courierId}`);
  ...
  async queryWaybillStatus(waybillNo: string): Promise<any> {
    try {
      const response = await this.httpClient.get(`/dispatch/waybill/${waybillNo}`);
      return response.data;
    } catch (error) {
  ...
  async reassignCourier(
    ...
    try {
      this.logger.log(`Reassigning order ${orderId} from ${oldCourierId} to ${newCourierId}`);
      ...
      this.logger.error(`Failed to reassign courier for order ${orderId}:`, error);
      throw error;
    }
  }
}
