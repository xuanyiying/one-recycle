...
export class InventoryServiceClient {
  ...
  constructor(private readonly configService: ConfigService) {
    this.baseURL = this.configService.get<string>('INVENTORY_SERVICE_URL') || 'http://localhost:3009';
  ...
  private setupInterceptors(): void {
    this.httpClient.interceptors.request.use(
      (config) => {
        this.logger.debug(`Inventory request: ${config.method?.toUpperCase()} ${config.url}`);
        return config;
      },
      (error) => {
  ...
  async lockInventory(request: InventoryLockRequest): Promise<boolean> {
    try {
      ...
      this.logger.error(`Failed to lock inventory for order ${request.orderId}:`, error);
      throw error;
    }
  }
  ...
  async deductInventory(orderId: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`Failed to deduct inventory for order ${orderId}:`, error);
      throw error;
    }
  }
}
