...
export class OrderServiceClient {
  ...
  constructor(private readonly configService: ConfigService) {
    this.baseURL = this.configService.get<string>('ORDER_SERVICE_URL') || 'http://localhost:3003';
  ...
  async updateOrderStatus(
    ...
    try {
      const response = await this.httpClient.patch<Order>(`/orders/${orderId}/status`, {
        status,
        ...metadata,
      });
      return response.data;
  ...
  async updateOrderAmount(orderId: string, totalAmount: number): Promise<Order> {
    try {
      const response = await this.httpClient.patch<Order>(`/orders/${orderId}`, {
        totalAmount,
      });
      return response.data;
    } catch (error) {
   ...
   * 分配快递员
   */
  async assignCourier(orderId: string, courierId: string, waybillNo?: string): Promise<Order> {
    try {
      const response = await this.httpClient.patch<Order>(`/orders/${orderId}/assign`, {
        courierId,
        waybillNo,
      });
      return response.data;
  ...
  async cancelOrder(orderId: string, reason: string): Promise<Order> {
    try {
      const response = await this.httpClient.post<Order>(`/orders/${orderId}/cancel`, {
        reason,
      });
      return response.data;
    } catch (error) {
  ...
  async isOrderPaid(orderId: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`Failed to check payment status for order ${orderId}:`, error);
      return false;
    }
  }
}
