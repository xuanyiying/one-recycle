...
export class AlipayProvider implements IPaymentProvider {
  ...
  private readonly idGenerator = new SnowflakeIdGenerator({ workerId: 4, datacenterId: 1 });
  ...
  async transfer(
    ...
    try {
      ...
      if (this.isDevelopment) {
        this.logger.warn('Development mode: simulating Alipay transfer success');
      ...
      if (transferResponse.code !== '10000') {
        return {
          success: false,
          message: transferResponse.sub_msg || transferResponse.msg || '转账失败',
  ...
  private getErrorMessage(error: any): string {
    if (error.response?.data) {
      const responseKey = Object.keys(error.response.data).find(key => key.endsWith('_response'));
      if (responseKey) {
