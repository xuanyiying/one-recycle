...
export class WeChatPayProvider implements IPaymentProvider {
  ...
  private readonly idGenerator = new SnowflakeIdGenerator({ workerId: 5, datacenterId: 1 });
  ...
  async transfer(
    ...
    try {
      ...
      if (this.isDevelopment) {
        this.logger.warn('Development mode: simulating WeChat transfer success');
  ...
  private getErrorMessage(error: any): string {
    if (error.response?.data) {
      return error.response.data.return_msg || error.response.data.err_code_des || error.message;
    }
