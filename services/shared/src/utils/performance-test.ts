...
function testGeneratorPerformance(
  ...
  console.log(`  ✅ 唯一性: ${ids.size}/${iterations} (${((ids.size / iterations) * 100).toFixed(2)}%)`);
...
async function testConcurrentPerformance(
  ...
  console.log(`  🚀 性能: ${result.idsPerSecond.toLocaleString()} IDs/秒`);
  console.log(`  💾 内存使用: ${(memoryUsed / 1024 / 1024).toFixed(2)}MB`);
  console.log(`  ✅ 唯一性: ${allIds.size}/${totalIterations} (${((allIds.size / totalIterations) * 100).toFixed(2)}%)`);
 ...
 * 压力测试 - 测试极限性能
 */
function stressTest(name: string, generator: () => string, duration: number = 5000): PerformanceResult {
  ...
  console.log(`  🚀 性能: ${result.idsPerSecond.toLocaleString()} IDs/秒`);
  console.log(`  💾 内存使用: ${(memoryUsed / 1024 / 1024).toFixed(2)}MB`);
  console.log(`  ✅ 唯一性: ${ids.size}/${count} (${((ids.size / count) * 100).toFixed(2)}%)`);
...
function generateReport(results: PerformanceResult[]): void {
  ...
  results.forEach(result => {
    ...
    console.log(`| ${result.name.padEnd(8)} | ${result.idsPerSecond.toLocaleString().padStart(13)} | ${memoryMB.padStart(13)} | ${uniqueness.padStart(10)} |`);
  });
  
  console.log('\n🏆 性能排行榜:');
  const sortedByPerformance = [...results].sort((a, b) => b.idsPerSecond - a.idsPerSecond);
  sortedByPerformance.forEach((result, index) => {
    console.log(`  ${index + 1}. ${result.name}: ${result.idsPerSecond.toLocaleString()} IDs/秒`);
  });
  
  console.log('\n💾 内存使用排行榜:');
  const sortedByMemory = [...results].sort((a, b) => a.memoryUsed - b.memoryUsed);
...
async function runPerformanceTests(): Promise<void> {
  ...
  try {
    ...
    results.push(testGeneratorPerformance('雪花算法', () => generateUniqueId()));
    results.push(testGeneratorPerformance('NanoID', () => generateShortId()));
    results.push(testGeneratorPerformance('订单号', () => generateSecureOrderNumber()));
    results.push(testGeneratorPerformance('支付单号', () => generateSecurePaymentNumber()));
    results.push(testGeneratorPerformance('雪花算法(直接)', () => snowflakeGenerator.nextId()));
    results.push(testGeneratorPerformance('NanoID(直接)', () => nanoIdGenerator.nextId()));
    results.push(testGeneratorPerformance('短NanoID', () => shortNanoIdGenerator.nextId()));
    
    // 并发性能测试
    console.log('\n🔄 并发性能测试');
    console.log('-'.repeat(50));
    
    results.push(await testConcurrentPerformance('雪花算法', () => generateUniqueId()));
    results.push(await testConcurrentPerformance('NanoID', () => generateShortId()));
    results.push(await testConcurrentPerformance('订单号', () => generateSecureOrderNumber()));
...
export { runPerformanceTests, testGeneratorPerformance, testConcurrentPerformance, stressTest };