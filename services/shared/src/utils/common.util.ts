...
export class SnowflakeIdGenerator {
  ...
  private static readonly MAX_DATACENTER_ID = (1 << SnowflakeIdGenerator.DATACENTER_ID_BITS) - 1;
  ...
  private static readonly DATACENTER_ID_SHIFT = SnowflakeIdGenerator.SEQUENCE_BITS + SnowflakeIdGenerator.MACHINE_ID_BITS;
  private static readonly TIMESTAMP_SHIFT = SnowflakeIdGenerator.SEQUENCE_BITS + SnowflakeIdGenerator.MACHINE_ID_BITS + SnowflakeIdGenerator.DATACENTER_ID_BITS;
  ...
  constructor(config: SnowflakeConfig = {}) {
    ...
    if (this.workerId > SnowflakeIdGenerator.MAX_MACHINE_ID || this.workerId < 0) {
      throw new Error(`Machine ID must be between 0 and ${SnowflakeIdGenerator.MAX_MACHINE_ID}`);
    }

    if (this.datacenterId > SnowflakeIdGenerator.MAX_DATACENTER_ID || this.datacenterId < 0) {
  ...
  public nextId(): string {
    ...
    const id = ((timestamp - this.epoch) << SnowflakeIdGenerator.TIMESTAMP_SHIFT) |
  ...
  public parseTimestamp(id: string): Date {
    ...
    const timestamp = Number(idNum >> BigInt(SnowflakeIdGenerator.TIMESTAMP_SHIFT)) + this.epoch;
...
export class NanoIdGenerator {
  private static readonly DEFAULT_ALPHABET = '_-0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
...
export class IdGeneratorFactory {
  private static instances = new Map<string, SnowflakeIdGenerator | NanoIdGenerator>();

  /**
   * 创建或获取ID生成器实例
   */
  public static getInstance(name: string, config: IdGeneratorConfig): SnowflakeIdGenerator | NanoIdGenerator {
    ...
    return IdGeneratorFactory.instances.get(name)!;
  }
  ...
  if (typeof obj === 'object' && obj !== null) {
    ...
    for (const key in obj as Record<string, any>) {
      if ((obj as Record<string, any>).hasOwnProperty(key)) {
        (clonedObj as Record<string, any>)[key] = deepClone((obj as Record<string, any>)[key]);
      }
    }
    return clonedObj;
  }
...
export class IdGeneratorMetrics {
  ...
  public static recordGeneration(generatorName: string, duration: number, success: boolean = true): void {
    const current = IdGeneratorMetrics.metrics.get(generatorName) || { count: 0, totalTime: 0, errors: 0 };
    current.count++;
  ...
  public static getMetrics(generatorName: string): { avgTime: number; errorRate: number; totalCount: number } | null {
...
export class IdValidator {
   ...
   * 验证订单号格式
   */
  public static validateOrderNumber(orderNumber: string, prefix: string = 'ORD'): boolean {
   ...
   * 验证支付单号格式
   */
  public static validatePaymentNumber(paymentNumber: string, prefix: string = 'PAY'): boolean {
