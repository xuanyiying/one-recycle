...
export class LogisticsService {
  ...
  async create(createDto: CreateLogisticsProviderDto) {
    this.logger.log(`Creating logistics provider: ${createDto.name} (${createDto.code})`);
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
        this.logger.warn(`Failed to create provider: code '${createDto.code}' already exists`);
        throw new ConflictException(`物流服务商编码 '${createDto.code}' 已存在`);
      }
      this.logger.error(`Failed to create logistics provider: ${error?.message}`, error?.stack);
      throw error;
    }
  }

  async findAll(params?: LogisticsProviderQueryParams): Promise<PaginatedResponse<any>> {
    const page = Math.max(1, params?.page || 1);
    const limit = Math.max(1, Math.min(100, params?.limit || 10));
    const skip = (page - 1) * limit;

    this.logger.log(`Fetching logistics providers: page=${page}, limit=${limit}`);
    ...
    try {
      ...
      this.logger.log(`Fetched ${providers.length} providers (total: ${total}, totalPages: ${totalPages})`);
      ...
      this.logger.error(`Failed to fetch logistics providers: ${error?.message}`, error?.stack);
      throw error;
    }
  }
  ...
  async findOne(id: number) {
    ...
    try {
      ...
      this.logger.log(`Found logistics provider: ${provider.name} (${provider.code})`);
      return provider;
    } catch (error: any) {
      if (error instanceof NotFoundException) {
        throw error;
      }
      this.logger.error(`Failed to find logistics provider ${id}: ${error?.message}`, error?.stack);
      throw error;
    }
  }
  ...
  async update(id: number, updateDto: UpdateLogisticsProviderDto) {
    ...
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
        this.logger.warn(`Failed to update provider ${id}: code '${updateDto.code}' already exists`);
        throw new ConflictException(`物流服务商编码 '${updateDto.code}' 已存在`);
      }
      this.logger.error(`Failed to update logistics provider ${id}: ${error?.message}`, error?.stack);
      throw error;
    }
  }
  ...
  async remove(id: number) {
    ...
    try {
      ...
      this.logger.error(`Failed to remove logistics provider ${id}: ${error?.message}`, error?.stack);
      throw error;
    }
  }

  async calculateFreight(weight: number, volume: number, distance: number) {
    this.logger.log(`Calculating freight: weight=${weight}, volume=${volume}, distance=${distance}`);
    try {
      const activeProviders = await this.prisma.logisticsProvider.findMany({
        where: { isActive: true },
      });

      this.logger.log(`Found ${activeProviders.length} active providers for freight calculation`);

      // 模拟返回首个可用服务商的价格
      if (activeProviders.length > 0) {
        const cost = 10 + weight * 2 + distance * 0.5;
        this.logger.log(`Freight calculated: provider=${activeProviders[0].name}, cost=${cost}`);
        return {
      ...
      this.logger.error(`Failed to calculate freight: ${error?.message}`, error?.stack);
      throw error;
    }
  }
}
