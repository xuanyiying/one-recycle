...
export class AddressParser {
  ...
  async parse(text: string): Promise<ParsedAddress> {
    ...
    // 4. 提取乡镇/街道
    const townPatterns = [/([^\s]+) 镇/, /([^\s]+) 乡/, /([^\s]+) 街道/, /([^\s]+) 路/];
    ...
    // 5. 提取详细地址
    const detailKeywords = ['小区', '号楼', '单元', '室', '户', '号', '栋', '楼'];
    ...
    if (detailStart < text.length) {
      ...
      // 清理详细地址中的省市区信息
      if (address.province) address.detail = address.detail.replace(address.province, '');
      if (address.city) address.detail = address.detail.replace(address.city, '');
      if (address.district) address.detail = address.detail.replace(address.district, '');
    } else {
      // 如果没有关键词，取最后一段作为详细地址
