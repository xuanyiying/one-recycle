...
export class CourierService implements OnModuleInit {
  ...
  async respondToNotification(
    ...
    const notification = await this.prisma.pickupNotification.findUnique({
    ...
    if (notification.courierId !== courierId) {
      throw new BadRequestException(
        'Courier not authorized for this notification',
      );
    }
  ...
  async findNotifications(filters?: any) {
    ...
    return this.prisma.pickupNotification.findMany({
      where,
  ...
  async getCourierPerformance(courierId: string, period: string) {
    ...
    const completedTasks = assignments.filter(
      (a) => a.status === ('DELIVERED' as any),
    ).length;
    const cancelledTasks = assignments.filter((a) =>
      ['CANCELLED', 'FAILED'].includes(String(a.status)),
    ).length;
    ...
    const averagePickupTime =
      durations.length > 0
        ? Math.floor(durations.reduce((s, v) => s + v, 0) / durations.length)
        : 0;
