...
export class CourierController {
  ...
  async findAllCouriers(
    ...
    return this.courierService.findAll(Object.keys(filters).length ? filters : undefined);
  }
  ...
  async findAvailableCouriers(
    ...
    let location;
    if (latitude && longitude && radius) {
      location = { latitude: Number(latitude), longitude: Number(longitude), radius: Number(radius) };
    }
  ...
  @Put(':id')
  async updateCourier(@Param('id') id: string, @Body() updateCourierDto: UpdateCourierDto) {
  ...
  @Get(':id/performance')
  async getCourierPerformance(@Param('id') id: string, @Query('period') period: string) {
    if (!period) {
  ...
  async getCourierAssignments(
    ...
    return this.courierService.findCourierAssignments(id, Object.keys(filters).length ? filters : undefined);
  }

  @Post('notifications')
  @HttpCode(HttpStatus.CREATED)
  async createPickupNotification(@Body() createNotificationDto: CreatePickupNotificationDto) {
  ...
  async findNotifications(
    ...
    if (startDate) filters.startDate = new Date(startDate);
    if (endDate) filters.endDate = new Date(endDate);
    return this.courierService.findNotifications(Object.keys(filters).length ? filters : undefined);
  }
  ...
  async respondToNotification(
    ...
    return this.courierService.respondToNotification(notificationId, responseData.courierId, responseData);
  }
  ...
  async updateTaskStatus(
    ...
    return this.courierService.updateTaskStatus(taskId, updateData.courierId, updateData);
  }
