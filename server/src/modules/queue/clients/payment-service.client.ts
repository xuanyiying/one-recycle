...
export class PaymentServiceClient {
  ...
  private setupInterceptors(): void {
    this.httpClient.interceptors.request.use(
      (config) => {
        this.logger.debug(
          `Payment request: ${config.method?.toUpperCase()} ${config.url}`,
        );
        return config;
      },
      (error) => {
  ...
  async increaseBalance(
    ...
    try {
      this.logger.log(
        `Increasing balance for user ${request.userId}, order ${request.orderId}, amount: ${request.amount}`,
      );
      ...
      this.logger.log(
        `Balance increased successfully for user ${request.userId}, new balance: ${response.data.balanceAfter}`,
      );
      return response.data;
    } catch (error) {
      this.logger.error(
        `Failed to increase balance for user ${request.userId}, order ${request.orderId}:`,
        error,
      );
      throw error;
    }
  ...
  async initiateRefund(request: RefundRequest): Promise<RefundResponse> {
    try {
      ...
      this.logger.error(
        `Failed to initiate refund for order ${request.orderId}:`,
        error,
      );
      throw error;
    }
  ...
  async getOrderPayment(orderId: string): Promise<any> {
    try {
      ...
      this.logger.error(
        `Failed to get payment info for order ${orderId}:`,
        error,
      );
      throw error;
    }
  ...
  async createPaymentLog(param: {
    ...
    try {
      await this.httpClient.post('/payments/log', param);
    } catch (error) {
      this.logger.error(
        `Failed to create payment log for transaction ${param.transactionId}:`,
        error,
      );
      throw error;
    }
  ...
  async isTransactionProcessed(transactionId: string) {
    try {
      const response = await this.httpClient.get(
        `/payments/log/${transactionId}`,
      );
      return response.data as unknown;
    } catch (error) {
      this.logger.error(
        `Failed to check if transaction ${transactionId} is processed:`,
        error,
      );
      throw error;
    }
