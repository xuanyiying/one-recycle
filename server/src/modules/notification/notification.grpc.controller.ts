...
export class NotificationGrpcController {
  ...
  async sendBatch(data: SendBatchRequest): Promise<SendBatchResponse> {
    const res = await this.notificationService.sendBatch(
      data.type,
  ...
  async updateTemplate(data: UpdateTemplateRequest): Promise<TemplateResponse> {
    const t = await this.notificationService.updateTemplate(
      data.id,
      data as any,
    );
    return {
  ...
  async listTemplates(
    ...
    const templates = await this.notificationService.findTemplates({
      type: data.type,
    } as any);
    return {
  ...
  async callbackStatus(
    data: CallbackStatusRequest,
  ): Promise<CallbackStatusResponse> {
    // 简化：仅记录回调
