...
import { NotificationFilters, TemplateFilters, BatchFilters, NotificationProvider } from '../interfaces/notification.interface';
...
export class NotificationController {
  ...
  async sendNotification(@Body() sendNotificationDto: SendNotificationDto): Promise<NotificationEntity> {
    ...
    const notificationData = {
      ...
      deliveryOptions: sendNotificationDto.deliveryOptions ? {
        ...
        expiresAt: sendNotificationDto.deliveryOptions.expiresAt ? new Date(sendNotificationDto.deliveryOptions.expiresAt) : undefined,
        scheduleAt: sendNotificationDto.deliveryOptions.scheduleAt ? new Date(sendNotificationDto.deliveryOptions.scheduleAt) : undefined,
  ...
  @Post('batch')
  @HttpCode(HttpStatus.CREATED)
  async sendBatchNotifications(@Body() sendBatchDto: SendBatchNotificationDto): Promise<NotificationBatchEntity> {
    ...
    const batchData = {
      ...
      notifications: sendBatchDto.notifications.map(dto => ({
        ...
        deliveryOptions: dto.deliveryOptions ? {
          ...
          expiresAt: dto.deliveryOptions.expiresAt ? new Date(dto.deliveryOptions.expiresAt) : undefined,
          scheduleAt: dto.deliveryOptions.scheduleAt ? new Date(dto.deliveryOptions.scheduleAt) : undefined,
  ...
  async findNotifications(
    ...
    const allNotifications = await this.notificationService.findNotifications(filters);
  ...
  @Get(':id')
  async findNotificationById(@Param('id') id: string): Promise<NotificationEntity> {
    return this.notificationService.findNotificationById(id);
  }

  @Post(':id/retry')
  @HttpCode(HttpStatus.OK)
  async retryNotification(@Param('id') id: string): Promise<NotificationEntity> {
  ...
  @Post(':id/cancel')
  @HttpCode(HttpStatus.OK)
  async cancelNotification(@Param('id') id: string): Promise<NotificationEntity> {
  ...
  @Post('templates')
  @HttpCode(HttpStatus.CREATED)
  async createTemplate(@Body() createTemplateDto: CreateTemplateDto): Promise<NotificationTemplateEntity> {
  ...
  @Get('templates/:id')
  async findTemplateById(@Param('id') id: string): Promise<NotificationTemplateEntity> {
  ...
  @Get('batches/:id')
  async findBatchById(@Param('id') id: string): Promise<NotificationBatchEntity> {
    return this.notificationService.findBatchById(id);
  }

  @Get('batches/:id/notifications')
  async getBatchNotifications(@Param('id') id: string): Promise<NotificationEntity[]> {
    return this.notificationService.getBatchNotifications(id);
  }

  @Get('stats/:period')
  async getNotificationStats(@Param('period') period: string): Promise<NotificationStatsEntity> {
  ...
  @Post('sms')
  @HttpCode(HttpStatus.OK)
  async sendSms(@Body() body: { phone: string; message: string }): Promise<{ success: boolean; messageId: string }> {
  ...
  @Get('health')
  @HttpCode(HttpStatus.OK)
  async healthCheck(): Promise<{ status: string; timestamp: string; providers: any }> {
