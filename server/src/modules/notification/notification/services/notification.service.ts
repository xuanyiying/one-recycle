...
export class NotificationService implements INotificationService {
  ...
  async sendNotification(data: SendNotificationData): Promise<any> {
    ...
    const notification = await this.prisma.notification.create({
      data: {
        ...
        content: typeof (data as any).content === 'string' ? (data as any).content : JSON.stringify((data as any).content),
        deliveryOptions: (data as any).deliveryOptions as any,
      }
    });
  ...
  async sendSms(data: { phone: string; template: string; params: Record<string, any> }): Promise<{ messageId: string }> {
    const messageId = this.generateMessageId();
    return { messageId };
  }

  async sendPush(data: { userId: string | number; title: string; content: string; data?: Record<string, any> }): Promise<{ success: boolean; devicesCount: number; successCount: number; sentAt: string }> {
    const devicesCount = 1;
    const successCount = 1;
    return { success: true, devicesCount, successCount, sentAt: new Date().toISOString() };
  }

  async sendEmail(data: { to: string; subject: string; content: string; template?: string; params?: Record<string, any> }): Promise<{ messageId: string }> {
    const messageId = this.generateMessageId();
    return { messageId };
  }

  async sendBatch(type: string, notifications: any[]): Promise<{ successCount: number; failedCount: number }> {
    let successCount = 0;
    let failedCount = 0;
  ...
  async findNotifications(filters?: NotificationFilters): Promise<any[]> {
    ...
    if (filters) {
      ...
      if (filters.userId) where.recipient = { path: ['userId'], equals: filters.userId };
      if (filters.batchId) where.deliveryOptions = { path: ['batchId'], equals: filters.batchId };
    }
  ...
  async getNotificationStats(period: string): Promise<NotificationStatsEntity> {
    ...
    // 计算各类型统计
    const breakdown: any = {
      sms: this.calculateTypeStats(periodNotifications as any, NotificationType.SMS),
      email: this.calculateTypeStats(periodNotifications as any, NotificationType.EMAIL),
      push: this.calculateTypeStats(periodNotifications as any, NotificationType.PUSH),
      inApp: this.calculateTypeStats(periodNotifications as any, NotificationType.IN_APP),
      webhook: this.calculateTypeStats(periodNotifications as any, NotificationType.WEBHOOK),
    };

    const costTotal = Object.values(breakdown as Record<string, { cost: number }>).reduce((sum, type) => sum + (type?.cost || 0), 0);
  ...
  private async processNotification(notification: any): Promise<void> {
    ...
    const updatedNotification = await this.prisma.notification.update({
      ...
      // 模拟发送过程
      const providers = await this.getProviders();
      const provider = providers.find(p => p.type === (updatedNotification.type as any));
      if (!provider || !provider.isEnabled) {
        throw new Error(`Provider for ${updatedNotification.type} is not available`);
      }
      ...
      this.logger.log(`Processed notification ${updatedNotification.id}: ${updatedNotification.status}`);
  ...
  private async processBatchNotifications(batch: any, notifications: any[]): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Batch processing failed for ${updatedBatch.id}: ${error.message}`);
    }
  }

  private calculateTypeStats(notifications: any[], type: NotificationType) {
    const typeNotifications = notifications.filter(n => String(n.type) === String(type));
    return {
      sent: typeNotifications.filter(n => n.status === NotificationStatus.SENT || n.status === NotificationStatus.DELIVERED).length,
      delivered: typeNotifications.filter(n => n.status === NotificationStatus.DELIVERED).length,
      failed: typeNotifications.filter(n => n.status === NotificationStatus.FAILED).length,
