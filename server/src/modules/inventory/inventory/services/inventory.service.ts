...
export class InventoryService {
  ...
  async createInventoryItem(data: CreateInventoryItemData): Promise<InventoryItemEntity> {
    ...
    const item = await this.prisma.inventoryItem.create({
      data: {
        ...
        sourceOrderId: data.sourceOrderId ? BigInt(data.sourceOrderId as any) : null,
        qualityGrade: data.qualityGrade,
        processingStatus: (data.processingStatus || ProcessingStatus.RECEIVED) as any,
  ...
  // 根据ID获取库存项目
  async getInventoryItemById(id: bigint): Promise<InventoryItemEntity | null> {
  ...
  async getInventoryStats(): Promise<InventoryStatsEntity> {
    const [totalItems, totalValue, lowStockItems, outOfStockItems, inStockItems] = await Promise.all([
    ...
    const averageValue = totalItems > 0 ? Number(totalValue._sum.totalPrice || 0) / totalItems : 0;
  ...
  // 创建交易记录
  async createTransaction(data: CreateTransactionData): Promise<InventoryTransactionEntity> {
  ...
  // 创建预订
  async createReservation(data: CreateReservationData): Promise<InventoryReservationEntity> {
  ...
  // 创建质量检查
  async createQualityCheck(data: CreateQualityCheckData): Promise<QualityCheckEntity> {
