...
export class CategoryService {
  ...
  async create(createCategoryDto: CreateCategoryDto): Promise<CategoryResponseDto> {
    ...
    const category = await this.prisma.category.create({
      data: {
        ...
        attributes: createCategoryDto.attributes ? JSON.stringify(createCategoryDto.attributes) : null
      }
    });
  ...
  async update(id: number, updateCategoryDto: UpdateCategoryDto): Promise<CategoryResponseDto> {
    ...
    // 如果更新了slug，检查是否已存在
    if (updateCategoryDto.seo?.slug && updateCategoryDto.seo.slug !== JSON.parse(existingCategory.seo).slug) {
    ...
    if (updateCategoryDto.type !== undefined) updateData.type = updateCategoryDto.type;
    if (updateCategoryDto.parentId !== undefined) updateData.parentId = updateCategoryDto.parentId;
    if (updateCategoryDto.priceInfo !== undefined) updateData.priceInfo = JSON.stringify(updateCategoryDto.priceInfo);
    if (updateCategoryDto.iconUrl !== undefined) updateData.icon = updateCategoryDto.iconUrl;
    if (updateCategoryDto.sortOrder !== undefined) updateData.sortOrder = updateCategoryDto.sortOrder;
    if (updateCategoryDto.isVisible !== undefined) updateData.isVisible = updateCategoryDto.isVisible;
    if (updateCategoryDto.isFeatured !== undefined) updateData.isFeatured = updateCategoryDto.isFeatured;
    if (updateCategoryDto.seo !== undefined) updateData.seo = JSON.stringify(updateCategoryDto.seo);
    if (updateCategoryDto.attributes !== undefined) updateData.attributes = updateCategoryDto.attributes ? JSON.stringify(updateCategoryDto.attributes) : null;

    // 如果更新了父分类，需要重新计算层级和路径
    if (updateCategoryDto.parentId !== undefined && updateCategoryDto.parentId !== existingCategory.parentId) {
      let level = 0;
      let path = '0';
  ...
  async findTree(parentId: number | null = null): Promise<CategoryResponseDto[]> {
