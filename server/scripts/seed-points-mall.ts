...
async function main() {
  ...
  try {
    ...
    for (const cfg of referralConfigs) {
      ...
      if (!existing) {
        ...
        console.log(`✅ 已创建返佣配置: ${cfg.key} = ${cfg.value}`);
      } else {
        console.log(`ℹ️ 返佣配置已存在: ${cfg.key}`);
      }
    }

    console.log('--- 积分商城数据初始化完成 ---');
  } catch (error) {
    console.error('❌ 初始化积分商城数据出错:', error);
    process.exit(1);
  } finally {
